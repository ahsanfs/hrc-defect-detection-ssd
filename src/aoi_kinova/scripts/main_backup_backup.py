...
class ButtonControllerNode:
    ...
    def update_combined_state(self):
        ...
        #     # Prevent reverting from 5 to 0
        #     if current == 5 and proposed == 0:
        #         continue  # keep as 5

        #     # Allow updates if current is 0, 5, or 7
        #     if current in [0, 5, 7]:
        #         self.final_state[i] = proposed
        #         self.locked[i] = proposed in [1, 2, 3, 4, 6]  # do not lock 5
        for i in range(len(self.buttons)):
            current = self.final_state[i]
        ...
        if self.last_uninspected_index is not None:
            ...
            if self.final_state[last_idx] == 7:
                ...
                if all_others_are_1:
                    ...
                    return  # <-- this return is needed here!

        # Step 3: Trigger waiting state ONLY if all are != 0 AND != 7
        if all(state != 0 and state != 5 and state != 7 for state in self.final_state):
            rospy.loginfo("All active states filled (no 0, 5 or 7) — entering reset wait mode.")
