...
labelsPath = os.path.sep.join([y_path, "objSSD.names"])
...
weightsPath = os.path.sep.join([y_path, "ssd_OK.weights"])
configPath = os.path.sep.join([y_path, "ssd_OK.cfg"])
...
ln = [ln[i - 1] for i in net.getUnconnectedOutLayers()]        # old code changed on Feb-4-2022
...
while True:
    ...
    if not win_started:
        ...
        window = sg.Window('Webcam Demo', layout, default_element_size=(14, 1), text_justification='right', auto_size_text=False, finalize=True)
