...
layout = 	[
		[sg.Text('Inspection')],
		[sg.Text('Path to image'), sg.In(r'images/image39.jpg',size=(40,1), key='image'), sg.FileBrowse()],
		[sg.Text('base path'), sg.In(r'config',size=(40,1), key='config'), sg.FolderBrowse()],
...
window = sg.Window('Inspection', layout,
...
labelsPath = os.path.sep.join([args["config"], "objSSD.names"])
#LABELS = open(labelsPath).read().strip().split("\n")
LABELS = open("/home/san/spain_taiwan_ws/src/inspection_system/script/objSSD.names").read().strip().split("\n")
...
weightsPath = os.path.sep.join([args["config"], "ssd_OK.weights"])
configPath = os.path.sep.join([args["config"], "ssd_OK.cfg"])

# load our YOLO object detector trained on COCO dataset (80 classes)
print("[INFO] loading from disk...")
net = cv2.dnn.readNetFromDarknet("/home/san/spain_taiwan_ws/src/inspection_system/script/ssd_OK.cfg", "/home/san/spain_taiwan_ws/src/inspection_system/script/ssd_OK.weights")
...
# Resize image to make it smaller (adjust scale_percent as needed)
