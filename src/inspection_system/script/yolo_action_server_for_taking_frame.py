...
class YoloWebcamActionServer:
    def __init__(self):
        ...
        self.labels_path = os.path.join(self.y_path, "objSSD.names")
        self.config_path = os.path.join(self.y_path, "ssd_OK.cfg")
        self.weights_path = os.path.join(self.y_path, "ssd_OK.weights")
    ...
    def execute(self, goal):
        ...
        base_dir = os.path.expanduser("~/yolo_detections")
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        session_dir = os.path.join(base_dir, f"session_{timestamp}")
        os.makedirs(session_dir, exist_ok=True)

        while not rospy.is_shutdown() and frame_counter < 10:
            ret, frame = cap.read()
            if not ret:
                break
            ...
            # Save frames before and after detection
            before_path = os.path.join(session_dir, f"frame_{frame_counter:02d}_before.jpg")
            after_path = os.path.join(session_dir, f"frame_{frame_counter:02d}_after.jpg")
            cv2.imwrite(before_path, raw_frame)
